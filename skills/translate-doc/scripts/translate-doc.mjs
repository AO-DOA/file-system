#!/usr/bin/env node
// translate-doc 生成器（translate-doc 技能的确定性配套）
//
...
//   源码路径键 = 工作区名 + '/' + 相对工作区根路径（根=工作区名）。仅翻译 md/markdown（宿主校验）。
//
// 职责：模型写好译文正文（忠实原文的中文译文，特殊名词首次出现用 原文（中文解释）），
// 本脚本只兜确定性——frontmatter（源码路径/层级: 文章翻译/生成时间）、自动命名（与 file-doc 同规则）、
// index.json「文章翻译」upsert。
//
// 命名规则（与 file-doc 一致，必然不重复）：文件名前缀 = 文件相对工作区根的完整父目录路径
// （原样保留全部层级，用 - 连）。相对路径唯一 → 文件名必然唯一。
//   例：dsh/README.md → dsh-README.md；顶层 README.md → <工作区名>-README.md。
//
// ⚠ 命名规则三处实现，改动必须三处同步，否则宿主 hasDocTr 判定与技能落盘文件名漂移：
//   ① src/host/fs-utils.ts computeDocStem
//   ② 本函数（skills/translate-doc/scripts/translate-doc.mjs computeName）
//   ③ skills/file-doc/scripts/file-doc.mjs computeName（同规则，同源）
//
// 用法：
//   node scripts/translate-doc.mjs gen <文件绝对路径> --book <书库根> --key <源码路径键> [--name <名字>] [--content <正文>]
...
// 如：dsh/README.md → dsh-README；顶层 README.md → <工作区名>-README。
...
function upsertIndex(book, entry, projectRoot) {
  ...
  for (const k of ['目录层', '文件层', '源码层', '文章翻译']) if (!Array.isArray(idx[k])) idx[k] = []
  const arr = idx['文章翻译']
...
if (mode === 'gen') {
  ...
  const outDir = join(book, '文章翻译')
  mkdirSync(outDir, { recursive: true })
  const name = argOf('--name') || computeName(srcKey, fileName)
  const outFile = `${name}.md`
  const override = argOf('--content')
  if (override == null) err('gen 需要 --content <译文正文>（模型写译文，脚本只兜 frontmatter/命名/索引）')
  // 模型只给译文正文：脚本前置拼 frontmatter（含生成时间），并剥掉正文里可能自带的旧头
  writeFileSync(join(outDir, outFile), buildFrontmatter(srcKey, '文章翻译') + stripFrontmatter(override), 'utf8')
  upsertIndex(book, { '源码路径': srcKey, '文档': `文章翻译/${outFile}` }, projectRoot)
  process.stdout.write(`\n已生成文章翻译 → ${join(outDir, name)}\n已 upsert index.json「文章翻译」: ${srcKey}\n`)
} else {
  err('用法: translate-doc <gen> ...')
}
