...
- `--menus=inline`：面板留在锚点旁（`.mr` root 内，`.mr-list` 是 `position:absolute`）⇒ 它是
  `.fs-hbar` / `.fs-hbar-mid` 的后代，**落进那两条 `overflow:hidden` 的裁剪链** —— 这是修复前
  （`Menu` 未传 `portal`）的真实形态。
...
| 运行 | 合计读数（3857 档） |
|---|---|
| 带 `--wsicon`（当前源码的**忠实**形态） | `gone 760` / `goneRight 504` / 右列最后被裁档 **306** |
...
未保存 / 编辑⇄保存（**合并成一个**按钮）。七个场景覆盖三列的各种「内容最宽」组合：

| id | 工作区名 | 中列 | 右列 | 用途 |
|---|---|---|---|---|
| `S0-empty` | 中（`dsh-plugin-file-system-zc`） | 无 | 只有分栏按钮 | 最空形态；面板 200–209 有 10 档右列部分裁切（已知） |
| `S1-source` | 中 | 视图「源码」+ 中路径 | 解读选择 + 未保存 + 保存（合并按钮） | 主力场景 |
| `S2-trlong` | **长** | 视图「文章翻译」+ 长路径 | 重新翻译 + 未保存 + 保存（合并按钮） | 最坏场景之一，下界通常由它决定 |
| `S3-dir` | 中 | 视图「目录概览」+ 中路径 | 只有解读选择 | 右列最简，用来对照 |
| `S4-trbusy` | 中 | 视图「源码」+ 长路径 | 翻译中…（**禁用态**）+ 未保存 + 保存（合并按钮） | 禁用按钮的几何与可点判定 |
| `S5-short` | 短（`dsh-fs`） | 视图「源码」+ 短路径 | 解读选择 + 编辑（合并按钮，无 `dirty`） | 其它维度的最短组合 |
...
- **探针只回答你让它问的问题 —— 交互态缺陷必须固化成新场景（用法纪律）**。场景是静态 DOM 复刻
  ...
  **全绿**的同时，页面上「悬停看不到下拉」：三个 `Menu` 默认内联渲染，面板是 `position:absolute`，
  而 `.fs-hbar` 与 `.fs-hbar-mid` 都带 `overflow:hidden`（为消跨列重叠加的兜底裁剪），面板作为它们的
