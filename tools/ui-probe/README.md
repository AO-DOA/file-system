...
| 运行 | 合计读数（3857 档） |
|---|---|
| 带 `--wsicon`（当前源码的**忠实**形态） | `gone 1010` / `goneRight 754` / 右列最后被裁档 **356** |
| 不带 `--wsicon`（工作区名永不收，左列常宽） | `gone **2013**` / `goneRight **1757**` / 右列最后被裁档 **576** |
...
- **当前已知的复刻滞后（2026-09-12，两处，`scenario()` 未同步）**：① 每个挂了气泡的顶栏按钮在源码里
  多了一层 `.fs-tipwrap` 锚点 span（`Tooltip` 要原生元素挂 ref）；② 编辑与保存已合并成一个按钮，
  复刻里仍是「编辑 + 保存」两个。**两处都实测过**（改 `SCEN` 里的场景 DOM、用同一段页面内脚本重跑
  3857 档）：① 逐档逐字段 **0 差异**（`--wsicon`、7 场景 × 551 档）；② 「少一个按钮」只会**变好**
  （被裁 1010→790、右列被整块裁下界 357→**307**，无任何一档变差）。所以现在的读数对①是等价的、
  对②是**保守**的（复刻多算了一个已经不存在的按钮）。下次动 `scenario()` 时一并把这两处补上，
  读数就会与源码形态一一对应。
