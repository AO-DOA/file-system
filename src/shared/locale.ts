...
export const ZH = {
  ...
  btnView: '查看',
  btnSave: '保存',
  // 分栏（R4）。可见文字沿用宿主 `dock.splitPane` 的「分栏」（与 §2 例外里那个字形同源，
  // 不自造术语）——用户裁决：全产品术语统一为「分栏」，此前的「分屏」写法一律废止。
  btnSplit: '分栏',
  // 气泡文案：必须是「功能名 + 最多一句极短限定」的短句，不能是整句说明。
  // 原先那句「分栏：把当前显示的视图复制一份只读副本到右侧；再点一次关闭」有 30 字，
  // 气泡被撑成一整条横在按钮下方，压住正文（用户截图反馈）。字数与「压住正文」是同一件事：
  // primitives 的气泡宽度不受调用方控制，只能靠缩短文案收窄。
  // 保留的两个信息是「右侧」（开在右边）与「只读副本」（不是共用同一份编辑态）；
  // 「再点一次关闭」的说明交给按钮恒定的 `aria-label` 与可见文字「分栏」承担，
  // 不再塞进气泡 —— 那是解释性内容，不是功能名。
  a11ySplit: '分栏：右侧只读副本',
