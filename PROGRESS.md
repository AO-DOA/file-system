...
（右侧显示 `/read` 失败文本，**不做自动处置**，且未在真实 host 复现）；B-3 切工作区后同 path 会被判成
「同对象」（**已知边界，不修**，见 §5 #15）；B-4 异对象支首次挂载多读一次 host（有意为之、已钉成断言）；
...
| 15 | 切工作区后**同名的相对 path 会被判成「同一个对象」** | **已知边界，不修**（R6 引入的新语义的副作用）。R6 的同一性判据是 `opened.path`（`/tree` 的相对路径），而 `refreshRoot`（切工作区 / 点刷新）会 `setOpened(null)` 并重读根树 —— 新 root 下若存在同名 path，左右会被判为同一对象而走「复用左侧 viewer」那一支。实际表现：切工作区后 `opened` 已被清空，右侧显示的是新 root 同名对象读到的东西，而不是冻结时那个。要修得给 `SplitTarget` 记 workspaceId / rootPath，属扩大改动面 ⇒ 本单不做，如实登记。**未在 GUI 里实测**（源码阅读 + 推理），详见 `docs/agent/reports/2026-09-13-split-freeze-target.md` §7 B-3 |
