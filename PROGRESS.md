...
| T-12 | README（对齐主仓结构，并须显著标注 G-1 高危与已知行为） | 进行中 | 子T12（`f248d86a`） | 交付 `README.md`（当前缺失，`package.json` 的 `files` 已声明）+ 源 README 功能点承接表（一条不漏） | - |
| T-14 | **陈旧引用修正 + `issues/` 文档迁移**（T-13 扫描发现的 11 条指向旧仓的引用） | 进行中 | 子T14（`6f848441`） | 用户口径「文档复制 + 改路径，技能全部连到新版本」；验收：`skills/` 的 git diff 中**只允许注释/文档行**变动（逻辑零改动自证）+ `docs/t14-stale-refs.md` 全仓扫描表 | - |
