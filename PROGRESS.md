...
| 8 | 顶栏窄档收纳（次要操作收进 `⋯` 菜单 / 折行） | **尚未完成**。为消解「面板 ≤480px 时右侧按钮（翻译 / 生成解读 / 编辑 / 保存）不可达」—— 本次 `.fs-hbar{overflow:hidden}` 修复**新引入的副作用**（右列按钮被裁档数 6/281 → **281/281**，可见可点 → 不可见不可点；判据与口径见 §3「顶栏 / 侧栏窄宽度重叠修复」表格第三行）。改法与实测口径见同节 |
| 9 | `.fs-genwrap` 造成的同列重叠 | **尚未完成**。**既有残留**（基线即有，非本次回归）：`.fs-genwrap{min-width:0}` 被 flex 压到 **20.3px**，其内 Button 渲染宽度 **52px**，「生成解读」溢出压住「● 未保存」；口径 B｜3507 档下基线 **1002 档** → 改后 **383 档**（S2 398–582 共 185 档 / S3 415–612 共 198 档），最大相交 **411px²**。口径定义与复跑方式见 §3 同节 |
